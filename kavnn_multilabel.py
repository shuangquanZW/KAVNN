...
from model import KAVNN_Multilabel


def train_ka_vnn(args: argparse.Namespace, organ: str | None = None):
    torch.set_float32_matmul_precision("medium")  # 平衡性能和精度
    # 数据模块
    dm = KAVNN_Multilabel.DataModule(
...
def load_parser(organ: str | None = None, random_state: int = 42):
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="KA_VNN_Multilabel")
    ...
    parser.add_argument("--num_labels", type=int, default=19, help="标签数量")
    ...
    return train_ka_vnn(parser.parse_args(), organ)


def main(organ: str | None = None, random_state: int = 42):
    _, precit_results = load_parser(organ, random_state)
    torch.save(precit_results, f"./multilabel/{organ}/kavnn/{random_state}.pt")
