...
def train_dnn(args: argparse.Namespace, organ: str | None = None, num_labels: int = 20):
    torch.set_float32_matmul_precision("medium")  # 平衡性能和精度
    # 数据模块
    dm = DNN.DNNDataModule(
    ...
    # 模型实例化
    model = DNN.DNN(num_neurals=args.num_neurals, num_labels=num_labels)
    # 训练器配置
    trainer = Trainer(
...
def load_parser(organ: str | None = None, random_state: int = 42, num_labels: int = 20):
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="DNN_Multilabel")
    ...
    return train_dnn(parser.parse_args(), organ, num_labels)
...
def main(organ: str | None = None, random_state: int = 42, num_labels: int = 20):
    ...
    with open(f"result/dnn_multilabel_{organ}_{random_state}.txt", "w") as f:
