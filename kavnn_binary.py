...
def train_ka_vnn(
    args: argparse.Namespace, organ: str | None = None, mode: str = "standard"
):
...
def load_parser(
    organ: str | None = None, random_state: int = 42, mode: str = "standard"
):
    # 解析命令行参数
    ...
    return train_ka_vnn(parser.parse_args(), organ, mode)


def main(organ: str | None = None, random_state: int = 42):
    _, precit_results = load_parser(organ, random_state)
    torch.save(precit_results, f"./binary/pt/{organ}/kavnn/{random_state}.pt")
