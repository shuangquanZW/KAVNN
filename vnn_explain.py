...
def get_attribute(model: VNN.VNN, comp_name: str) -> dict[str, dict[int, float]] | None:
    node_importance = model.explain(comp_name)
    return node_importance


def get_train_model() -> VNN.VNN:
    # 解析命令行参数
    ...
    parser.add_argument("--devices", type=list, default=[1], help="使用的设备数量")
...
def main():
    ...
    for drug in liver_drugs:
        node_attribute = get_attribute(model, drug)
        with open(f"./explain/VNN/{drug}_node_attribute.pkl", "wb") as f:
