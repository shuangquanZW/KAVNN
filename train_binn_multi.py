...
from dl.BINN import BINNMultilabel


def train_binn(args: argparse.Namespace, organ: str | None = None):
    torch.set_float32_matmul_precision("medium")  # 平衡性能和精度
    # 数据模块
    dm = BINNMultilabel.BINNDataModule(
...
def load_parser(organ: str | None = None, random_state: int = 42):
    ...
    return train_binn(parser.parse_args(), organ)


def main(organ: str | None = None, random_state: int = 42):
    _, precit_results = load_parser(organ, random_state)
    torch.save(precit_results, f"./binary/pt/{organ}/binn/{random_state}.pt")
