...
def train_dnn(args: argparse.Namespace, organ: str | None = None):
    ...
    # 模型实例化
    model = AttRethinkNetMultilabel.AttRethinkNetTrainer(num_labels=19)
    # 训练器配置
    trainer = Trainer(
...
def main(organ: str | None = None, random_state: int = 42):
    ...
    torch.save(precit_results, f"./binary/pt/{organ}/att_net/{random_state}.pt")
