...
def train_vnn(args: argparse.Namespace, organ: str | None = None, num_labels: int = 20):
...
def load_parser(organ: str | None = None, random_state: int = 42, num_labels: int = 20):
    ...
    return train_vnn(parser.parse_args(), organ, num_labels)
...
def main(organ: str | None = None, random_state: int = 42, num_labels: int = 20):
    ...
    with open(f"result/vnn_multilabel_{organ}_{random_state}.txt", "w") as f:
